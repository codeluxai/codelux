...
Prefer to build for yourself, or just want the checksum for a release you
already downloaded? Every asset here on GitHub ships with a `.sha256` file —
verify with `sha256sum -c` before running.
...
### Git
- **Branches Viewer** — see and switch branches, inspect HEAD, right from the dashboard.
- **Commit & Diff Viewer** — browse history and diffs with syntax highlighting; stage, commit and (once authorised) push, without leaving your AI workflow.
...
Signed release binaries only — the Free build of Codelux, published straight
from [codelux.ai](https://codelux.ai). No source code here: see
[codelux.ai/legal](https://codelux.ai/legal) for licensing terms.
